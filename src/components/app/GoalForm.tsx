...
import type { Goals as Goal, Milestone } from "@/types/index"
...
const GoalForm: React.FC<GoalFormInterface> = ({ goal, onClose }) => {
  ...
  const handleAddMilestoneToEditedGoal = () => {
    setEditedGoal((prev) => ({
      ...prev,
  ...
  return !goal ? (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <Label htmlFor="description" className="text-[#101422] dark:text-[#A8DCE7]">
          Description
        </Label>
        <Textarea
          id="description"
      ...
      <div>
        <Label htmlFor="category" className="text-[#101422] dark:text-[#A8DCE7]">
          Category
        </Label>
        <Input
          id="category"
      ...
      <div>
        <Label htmlFor="priority" className="text-[#101422] dark:text-[#A8DCE7]">
          Priority
        </Label>
      ...
      <div>
        <Label htmlFor="targetDate" className="text-[#101422] dark:text-[#A8DCE7]">
          Target Date
        </Label>
        <Input
          id="targetDate"
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-[#101422] dark:text-[#A8DCE7]">Milestones</h3>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            className="bg-[#FFFFFF] dark:bg-[#272B3B] text-[#101422] dark:text-[#A8DCE7] border-[#A8DCE7] dark:border-[#101422]"
          />
          <Select value={newMilestone.status} onValueChange={(status) => setNewMilestone({ ...newMilestone, status })}>
            <SelectTrigger className="bg-[#FFFFFF] dark:bg-[#272B3B] text-[#101422] dark:text-[#A8DCE7] border-[#A8DCE7] dark:border-[#101422]">
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <label className="text-sm font-semibold text-[#101422] dark:text-[#A8DCE7] mb-1 block">Category</label>
          <Input
            name="category"
        ...
        <div>
          <label className="text-sm font-semibold text-[#101422] dark:text-[#A8DCE7] mb-1 block">Priority</label>
          <Select
            name="priority"
        ...
        <div>
          <label className="text-sm font-semibold text-[#101422] dark:text-[#A8DCE7] mb-1 block">Status</label>
          <Select
            name="status"
            value={editedGoal.status}
            onValueChange={(value) => setEditedGoal((prev) => ({ ...prev, status: value as Status }))}
          >
            <SelectTrigger className="bg-[#FFFFFF] dark:bg-[#272B3B] text-[#101422] dark:text-[#A8DCE7] border-[#A8DCE7] dark:border-[#101422]">
              <SelectValue placeholder="Select milestone status" />
            </SelectTrigger>
        ...
        <div>
          <label className="text-sm font-semibold text-[#101422] dark:text-[#A8DCE7] mb-1 block">Target Date</label>
          <Input
            type="date"
            name="targetDate"
      ...
      <div>
        <label className="text-sm font-semibold text-[#101422] dark:text-[#A8DCE7] mb-2 block">Milestones</label>
        {editedGoal.milestones.map((milestone, index) => (
          <div
            key={index}
            className="space-y-2 mb-4 p-4 border border-[#A8DCE7]/30 dark:border-[#101422]/30 rounded-lg"
          >
            <div className="flex items-center space-x-2">
              <Input
                value={milestone.title}
                onChange={(e) => {
                  const updatedMilestones = editedGoal.milestones.map((item, idx) => {
                    if (idx === index) {
              ...
              <Input
                type="date"
                value={new Date(milestone.targetDate).toISOString().split("T")[0]}
                onChange={(e) => {
                  const updatedMilestones = [...editedGoal.milestones]
                  updatedMilestones[index].targetDate = new Date(e.target.value)
                  setEditedGoal((prev) => ({
                    ...prev,
                    milestones: updatedMilestones,
                  }))
                ...
                onValueChange={(value) => {
                  setEditedGoal((prev) => ({
                    ...
                    milestones: prev.milestones.map((milestone, idx) =>
                      idx === index ? { ...milestone, status: value } : milestone,
                    ),
                  }))
                }}
              >
            ...
            <div className="flex items-center space-x-2">
              <label className="text-sm font-medium text-[#101422] dark:text-[#A8DCE7]">Every Day Action</label>

              <Input
                type="checkbox"
                checked={milestone.everyDayAction !== undefined ? milestone.everyDayAction : false}
                onChange={() => {
              ...
              {milestone.status === "complet" && (
                <p className="text-xs text-[#FF0000]">
                  You cannot modify the 'Every Day Action' when the milestone is complete.
                </p>
              )}
            </div>
          </div>
        ...
        <div className="mt-6 p-4 border-2 border-dashed border-[#A8DCE7]/50 dark:border-[#101422]/50 rounded-lg space-y-3">
          <h4 className="text-sm font-semibold text-[#101422] dark:text-[#A8DCE7] flex items-center gap-2">
          ...
          <Button
            type="button"
            onClick={handleAddMilestoneToEditedGoal}
            className="w-full bg-[#A8DCE7] text-[#101422] hover:bg-[#272B3B] hover:text-[#A8DCE7] rounded-full transition-colors duration-200"
          >
